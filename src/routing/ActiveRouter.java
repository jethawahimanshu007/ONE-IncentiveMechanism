...
public abstract class ActiveRouter extends MessageRouter {
	...
	public boolean requestDeliverableMessages(Connection con) {
		...
		//System.out.println("Host" + getHost()+" has messages for "+other.getAddress()+":");
		for (Message m : temp) {
			if (other == m.getTo()) {
				//System.out.println("Trying to call startTransfer here.");
	...
	public int receiveMessage(Message m, DTNHost from) {
		...
		double thisMesRating=0.0;
		if (recvCheck != RCV_OK) {
			thisMesRating=Math.random()*m.MAX_RATING;
			//m.ratingMap.add(address,thisMesRating);
			DTNHost source=m.getFrom();
			
			//m.getTo().hostRatings.put(source, sum);
			return recvCheck;
		}
	...
	public int receiveMessage(Message m, DTNHost from,DTNHost to) {
		int recvCheck = checkReceiving(m, from);
		double thisMesRating=0.0;
		if (recvCheck != RCV_OK) {
			thisMesRating=Math.random()*m.MAX_RATING;
			//m.ratingMap.add(address,thisMesRating);
			DTNHost source=m.getFrom();
			ArrayList<Message> messagesInHost=new ArrayList<Message>(getMessageCollection());
			double sum=0.0;
			int nrOfMesFromSource=0;
			for(Message tempMes:messagesInHost)
			{
				if(tempMes.getFrom()==source)
				{
					//sum+=tempMes.rating;
					nrOfMesFromSource++;
				}
			}
			sum=sum/nrOfMesFromSource;
			m.getTo().hostRatings.put(source, sum);
			return recvCheck;
		}
	...
	/**For incrementing number of reduced messages to print in the log
	 * calls the function in MessageRouter
	 */
	public void incrementReducedMessagesToSelfish(){
		super.incrementReducedMessages();
	}
	@Override
	...
	protected Tuple<Message, Connection> tryMessagesForConnected(
		...
		for (Tuple<Message, Connection> t : tuples) {
			Message m = t.getKey();
			//System.out.println("Message being transferred is:"+m.getId());
			Connection con = t.getValue();
			count++;
			if (startTransfer(m, con) == RCV_OK) {
				//System.out.println("Number before returning a tuple is:"+count);
				return t;
			}
		}
	...
	protected Connection tryAllMessagesToAllConnections(){
		...
		//System.out.println("Messages this node's router has for transfer are:"+this.getNrofMessages());
