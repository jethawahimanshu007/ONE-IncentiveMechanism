...
public abstract class MessageRouter {
	...
	/** The messages this routfer is carrying */
	...
	/**Added function for incrementing number of reduced messages
	 * to selfish nodes for printin in the report
	 */
	
	public void incrementReducedMessages()
	{
		for (MessageListener ml : this.mListeners) {
			ml.incrementReducedMessagesToSelfish();
		}
	}
	/**
	...
	public int receiveMessage(Message m, DTNHost from,DTNHost to) {
		...
		m.ratingMap.put(to.getAddress(),thisMesRating);
		...
		{
			if(tempMes.getFrom()==from)
			{
				sum+=(Double)(tempMes.ratingMap.get(from.getAddress()));
				nrOfMesFromSource++;
			}
		}
		sum=sum/nrOfMesFromSource;
		to.hostRatings.put(from,sum);
