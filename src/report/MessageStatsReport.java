...
public class MessageStatsReport extends Report implements MessageListener {
	...
	private int nrOfCreatedForNonSelfish=0,nrOfDeliveredToNonSelfish=0,nrOfReducedToSelfish=0;
	...
	public void messageTransferred(Message m, DTNHost from, DTNHost to,
		...
		if (finalTarget) {
			...
			if(to.getSelfishFlag()==0)
			{
				nrOfDeliveredToNonSelfish++;
			}
			this.nrofDelivered++;
	...
	public void newMessage(Message m) {
		//System.out.println("New Message function is called!");
		if (isWarmup()) {
			addWarmupID(m.getId());
			return;
		}
		...
		if(m.getTo().getSelfishFlag()==0)
			nrOfCreatedForNonSelfish++;
	...
	public void done() {
		write("Message stats for scenario " + getScenarioName() +
				"\nsim_time: " + (format(getSimTime()/(1000*60))+" minutes"));
		...
		String statsText = "created: " + this.nrofCreated +
			...
			"\nNew things:\n\n"+
			//"\nNoOfCreatedForNonSelfish:"+nrOfCreatedForNonSelfish+
			"\nNrOfDeliveredToNonSelfish:"+nrOfDeliveredToNonSelfish+
			"\nTotal Messages reduced because of zero incentive:"+nrOfReducedToSelfish;
//nrOfCreatedForNonSelfish=0,nrOfDeliveredToNonSelfish=0,nrOfReducedToSelfish=0
		write(statsText);
		super.done();
  	}
