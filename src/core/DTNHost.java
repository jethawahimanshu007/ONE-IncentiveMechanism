...
public class DTNHost implements Comparable<DTNHost> {
	...
	private int selfishFlag=0;
	public int rank=1;
	private double incentiveOnHost=20;
	public ArrayList<String> interests;
	public HashMap<DTNHost,Double> hostRatings;
	public ArrayList<String> messageIds=new ArrayList<String>();
	public HashMap<String,Double> messageIdToRate =new HashMap<String,Double>();
	static {
	...
	public DTNHost(List<MessageListener> msgLs,
			...
			String groupId, List<NetworkInterface> interf,
			ModuleCommunicationBus comBus,
			MovementModel mmProto, MessageRouter mRouterProto,int SelfishFlag,ArrayList<String> interests,int rank) {
		this.comBus = comBus;
		this.location = new Coord(0,0);
		this.address = getNextAddress();
		this.name = groupId+address;
		...
		this.net = new ArrayList<NetworkInterface>();
		this.selfishFlag=SelfishFlag;
		this.rank=rank;
		hostRatings=new HashMap();
		for (NetworkInterface i : interf) {
			NetworkInterface ni = i.replicate();
			ni.setHost(this);
			net.add(ni);
	...
	public int receiveMessage(Message m, DTNHost from) {
		//System.out.println("Recieve message has been called by DTNHost!");
		//int retVal = this.router.receiveMessage(m, from);
		
		int retVal = this.router.receiveMessage(m, from,this);
		if (retVal == MessageRouter.RCV_OK) {
			m.addNodeOnPath(this);	// add this node on the messages path
			//Add to message list
			messageIds.add(m.getId());
			//Rate the received message
			messageIdToRate.put(m.getId(),Math.min(Math.random()*5.0+1.0,5.0));
		}
		
		double rateHost=0.0;
		int noOfMessagesFromFromNode=0;
		for(int i=0;i<messageIds.size();i++)
		{
			if(m.getFrom().getAddress()!=this.getAddress() && m.getFrom().getAddress()==from.getAddress()) {
				//System.out.println("MessageId in question in receiveMessage in DTNHost is:"+messageIds.get(i));
				rateHost+=messageIdToRate.get(messageIds.get(i));
				noOfMessagesFromFromNode++;
			}
		}
		//System.out.println("The value of ");
		rateHost=rateHost/(noOfMessagesFromFromNode*1.0);
		if(rateHost>5.0) 
		rateHost=5.0;
		hostRatings.put(from, rateHost);
		return retVal;
	}
	...
	public int getSelfishFlag(){
		return selfishFlag;
	}
	public double getIncentive(){
		return incentiveOnHost;
	}
	public void setIncentive(double incInput){
		incentiveOnHost=incInput;
	}
	public void updateRating(DTNHost host)
	{
