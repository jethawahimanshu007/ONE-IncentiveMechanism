...
public class SimScenario implements Serializable {
	...
	private double PERCENT_SELFISH_NODES=20.0;
	...
	protected void createHosts() {
		...
		for (int i=1; i<=nrofGroups; i++) {
			...
			///Determine the selfish nodes from the percentSelfishNodes parameter set earlier
			int totalSelfishNodes=(int)(PERCENT_SELFISH_NODES/100 * nrofHosts);
			System.out.println("Total selfish nodes:"+totalSelfishNodes);
			Random rand=new Random();
			ArrayList<Integer> selfishNodes=new ArrayList<Integer>();
			for(int countSelfish=0;countSelfish<totalSelfishNodes;countSelfish++)
			{
				while(true)
				{
				int randInt=rand.nextInt(nrofHosts);
				if(!selfishNodes.contains(randInt))
				{
					selfishNodes.add(randInt);
					break;
				}
				}
			}
			
			String[] interestSet= {"fire","soldier","parking-lot","tank","enemy","bomb","missile","arms","syria","building","pit","combat",
					"window","door","base","air-force","aircraft","jet","bullets","Omar-Abdullah","concert","sniper-rifle","gourmet cooking","hiking"
					,"light","pizza coupons","traffic accident","red ferrari","green lamborghini","starbucks coffee","samsung tab","chair","computer",
					"Mac","Dell","controller","conditioner","sun glasses","key","seat belt","clothes","video games","camera","beef","toothpaste","flowers",
					"book","bottle","tire swing","phone","magnet","power plant","knife","vase","newspaper","shoes","hanger","car","towel","perfume",
					"tree","house","desk","washing machine","stop sign","beer"};
			// creates hosts of ith group
			for (int j=0; j<nrofHosts; j++) {
				//Assign ranks for host
				int rank=1;
				if(j<=nrofHosts/3)
					rank=1;
				else if(j>=nrofHosts/3 && j<=2*nrofHosts/3)
					rank=2;
				else rank=3;
				ModuleCommunicationBus comBus = new ModuleCommunicationBus();
				DTNHost host;
				int flagForSelfish=0;
				// prototypes are given to new DTNHost which replicates
				// new instances of movement model and message router
				if(selfishNodes.contains(j))
				{
					flagForSelfish=1;
				}
				//Generate an arraylist of random interests for a host
				int noOfInterests = ThreadLocalRandom.current().nextInt(1, 10);
				//int noOfInterests=(int)(Math.random()*8);
				ArrayList<String> interestsForTheHost=new ArrayList<String>();
				ArrayList<Integer> interestIndices=new ArrayList<Integer>();
				for(int cIn=0;cIn<noOfInterests;cIn++)
				{
					
					int randomInterestIndex=(int)(Math.random()*interestSet.length);
					if(!interestIndices.contains(randomInterestIndex))
						{
							interestIndices.add(randomInterestIndex);
							interestsForTheHost.add(interestSet[randomInterestIndex]);
							//System.out.print("Adding the interest "+interestSet[randomInterestIndex]+" to the host!");
						}
				}
				
				System.out.println("Interests generated for the host are:"+interestsForTheHost);
				host = new DTNHost(this.messageListeners,
						this.movementListeners,	gid, interfaces, comBus,
						mmProto, mRouterProto,flagForSelfish,interestsForTheHost,rank);
				hosts.add(host);
			}
			System.out.print("These selfish nodes are:");
			for(int selC=0;selC<hosts.size();selC++)
			{
				if(hosts.get(selC).getSelfishFlag()==1)
				System.out.print("\t"+selC);
			}
			System.out.println();
		}
	}
