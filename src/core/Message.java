...
public class Message implements Comparable<Message> {
	...
	private int initTtl;
	public double MAX_RATING=5.0;
	public HashMap ratingMap=new HashMap<Integer,Double>();
	//public double rating=MAX_RATING;
	...
	private double incentivePromise=0.0;
	...
	public Message(DTNHost from, DTNHost to, String id, int size) {
		...
		this.appID = null;
		//System.out.println("interests size for the from node is"+from.interests.size());
		int noOfTags = ThreadLocalRandom.current().nextInt(1, from.interests.size() + 1);
		//int noOfTags=(int)(Math.random()*from.interests.size());
		//System.out.println("Number of tags for the message are:"+noOfTags);
		ArrayList<Integer> interestIndices=new ArrayList<Integer>();
		tags=new ArrayList<String>();
		for(int cIn=0;cIn<noOfTags;cIn++)
		{
			int randomInterestIndex=(int)(Math.random()*from.interests.size());
			if(!interestIndices.contains(randomInterestIndex))
				{
					interestIndices.add(randomInterestIndex);
					tags.add(from.interests.get(randomInterestIndex));
				}
		}
		Message.nextUniqueId++;
		addNodeOnPath(from);
		sizePercent=Math.random();
		qualityPercent=Math.random();
	}
	...
	public void setIncentivePromise(double promise){
		incentivePromise=promise;
	}
	public double getIncentivePromise(){
		return incentivePromise;
	}
